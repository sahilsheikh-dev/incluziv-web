<template>
  <footer class="footer navbar py-md-5">
    <div class="container">
      <div class="row my-0">
        <div
          class="col-md-3 col-lg-3 col-xl-3 text-start offset-md-2 offset-lg-2 offset-xl-2"
        >
          <img
            class="my-3"
            src="../../assets/logo.png"
            style="max-width: 120px;"
          />
          <div class="row">
            <div class="col-1">
              <a href="https://facebook.com" class="text-decoration-none">
                <img
                  class="img-thumbnail bg-transparent mx-1"
                  src="../../assets/img/facebook.png"
                  alt="logo"
                  style="max-width: 30px;"
                />
              </a>
            </div>
            <div class="col-1">
              <a href="https://facebook.com" class="text-decoration-none">
                <img
                  class="img-thumbnail bg-transparent mx-1"
                  src="../../assets/img/twitter.png"
                  alt="logo"
                  style="max-width: 30px;"
                />
              </a>
            </div>
            <div class="col-1">
              <a href="https://facebook.com" class="text-decoration-none">
                <img
                  class="img-thumbnail bg-transparent mx-1"
                  src="../../assets/img/instagram.png"
                  alt="logo"
                  style="max-width: 30px;"
                />
              </a>
            </div>
            <div class="col-1">
              <a href="https://facebook.com" class="text-decoration-none">
                <img
                  class="img-thumbnail bg-transparent mx-1"
                  src="../../assets/img/linkedin.png"
                  alt="logo"
                  style="max-width: 30px;"
                />
              </a>
            </div>
          </div>
        </div>
        <div class="col-md-3 col-lg-3 col-xl-3 text-start">
          <h4 class="fw-bold">Quick Links</h4>
          <a class="text-decoration-none text-dark" href="#/">Home</a> <br />
          <a class="text-decoration-none text-dark" href="#/jobseakers"
            >Job Seakers</a
          >
          <br />
          <a class="text-decoration-none text-dark" href="#/employers"
            >Employers</a
          >
          <br />
          <a class="text-decoration-none text-dark" href="#/community"
            >Community</a
          >
          <br />
          <a class="text-decoration-none text-dark" href="#/aboutus"
            >About Us</a
          >
          <br />
          <a class="text-decoration-none text-dark" href="#/contactus"
            >Contact Us</a
          >
          <br />
        </div>
        <div class="col-md-3 col-lg-3 col-xl-3 text-start">
          <h4 class="fw-bold">Contact Us</h4>
          <a
            class="text-decoration-none text-dark"
            href="https://goo.gl/maps/zvJd7KM1Uv5oyqB38"
            target="_blank"
          >
            <i class="bi bi-geo-alt-fill text-primary"></i>
            182 Gurgaon, Haryana - 122001
          </a>
          <br />
          <a class="text-decoration-none text-dark" href="tel:+91 5698784525">
            <i class="bi bi-telephone-fill text-primary"></i>
            +91 5698784525
          </a>
          <br />
          <a
            class="text-decoration-none text-dark"
            href="mailto:email@incluziv.com"
          >
            <span class="text-primary fw-bold">@</span>
            email@incluziv.com
          </a>
        </div>
      </div>
    </div>
    <div class="container">
      <div class="bg-dark" style="width: 100%; height: 2px;"></div>
    </div>
  </footer>
</template>

<style>
@import "../../assets/css/main.css";
</style>
